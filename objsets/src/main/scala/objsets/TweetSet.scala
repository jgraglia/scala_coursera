...
object GoogleVsApple {
  ...
  def inList(text:String, list:List[String]):Boolean = {
    !list.dropWhile(item => !text.contains(item)).isEmpty
    //list.map(item => text.contains(item)).contains(true)
  }
  lazy val googleTweets: TweetSet = {
    TweetReader.allTweets.filter(p => inList(p.text, google))
  }
  lazy val appleTweets: TweetSet = {
    TweetReader.allTweets.filter(p => inList(p.text, apple))
  }
