...
class ListsSuite extends FunSuite {
  ...
  test("sum of null list is ... illegal") {
    intercept[NullPointerException] {
      sum(null)
    }
  }
  test("sum of singleton is self") {
    assert(sum(List(4)) === 4)
  }

  test("sum of negative singleton is self") {
    assert(sum(List(-24)) === -24)
  }

  test("sum of 2 items") {
    assert(sum(List(3, 8)) === 11)
  }
  test("sum of negative items") {
    assert(sum(List(-3, -8)) === -11)
  }
  ...
  test("max of empty list is ... illegal") {
    intercept[NoSuchElementException] {
      max(List())
    }
  }
  test("max of null list is ... illegal") {
    intercept[NullPointerException] {
      max(null)
    }
  }
  test("max of a singleton is the singleton") {
    assert(max(List(9)) === 9)
  }

  test("max of a negative singleton is still the negative singleton") {
    assert(max(List(-9)) === -9)
  }

  test("max of a negative singleton with 0 is 0") {
    assert(max(List(-9, 0)) === 0)
  }

  test("max with same number") {
    assert(max(List(-9, -9, -9, -9)) === -9)
  }

  test("max of a mixed positive and negative ints") {
    assert(max(List(12, -9, 0, 23, -89)) === 23)
  }
}
